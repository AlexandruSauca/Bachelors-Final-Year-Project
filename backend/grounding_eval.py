...
REMINDER: If the answer is not in the context above, you MUST say "The provided context does not contain enough information." Do not use external knowledge.
...
"""
    ...
    generated_ids = model.generate(**inputs, max_new_tokens=400, temperature = 0.01, repetition_penalty=1.15)
...
def condense_query(user_query: str, history: list) -> str:
    ...
    condense_prompt = f"""You are a query rewriting system. Your ONLY job is to rewrite the latest User question so it can be understood without the chat history.
DO NOT answer the question. DO NOT add filler text. Output ONLY the rewritten question.
...
Rewritten Standalone Question:"""
    ...
    print("\n[Condenser Prompt] Sending to Qwen...")
    ...
    try:
        ...
        standalone_query = processor.batch_decode(generated_ids_trimmed, skip_special_tokens=True)[0]

        
        if not standalone_query or standalone_query.strip() == "":
            return user_query
            
        return standalone_query.strip()
        
    except Exception as e:
        print(f"⚠️ Condenser failed: {e}. Falling back to original query.")
        return user_query
