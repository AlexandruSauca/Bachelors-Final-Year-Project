...
output_qwen2 = "./results/gemini-2.5-flash-lite.jsonl"
...
def evaluate_mmlb(files, results_path, data_root,model, prompt):
    results = []

    for file in files:
        with open(file, 'r', encoding='utf-8') as f:
            for i,line in enumerate(tqdm(f, desc=f"Processing {os.path.basename(file)}")):
                # if i >= 5: # Only process 10 items
...
def main():
    ...
    results = evaluate_mmlb(files, output_qwen2, data_root, model, prompt_eval)
    ...
    if all_scores:
        ...
        print("\n---")
        print(f"Final Overall Average Score: {final_average:.2f} / 5.0")   
    else:
    ...
    if all_rouge:
        ...
        print("\n---")
        print(f" > Final Average RougeLSum Score: {final_rouge_100:.2f}") 
    else:
    ...
    plt.title(f"Evaluation Results for Gemini-2.5-Flash-Lite (Scores 0-100)")
    plt.ylabel("Score")
    ...
    plt.tight_layout() 
    plt.savefig("Gemini-2.5-Flash-Lite.png")
    plt.show()
